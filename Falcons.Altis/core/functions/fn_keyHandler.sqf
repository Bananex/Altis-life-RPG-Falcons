			...
			if(_veh isKindOf "House_F" && playerSide == civilian) then {
				...
				if(_veh in life_vehicles && player distance _veh < 8) then {
					if(_locked == 2) then {
						...
						[[player, "car_unlock",10],"life_fnc_playSound",true,false] spawn life_fnc_MP;
						hint composeText [ image "icons\unlock.paa", "  Véhicule dévérouiller" ];
					} else {
						if(local _veh) then {
							_veh lock 2;
						} else {
						...
						[[player, "car_unlock",10],"life_fnc_playSound",true,false] spawn life_fnc_MP;
						hint composeText [ image "icons\lock.paa", "  Véhicule vérouiller" ];
					};
				};
			};
		};
